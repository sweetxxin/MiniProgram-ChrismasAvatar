...
dragGraph.prototype = {
     ...
     */
    paint() {
      var avatarurl_width = 128;    //绘制的头像宽度
      var avatarurl_heigth = 128;   //绘制的头像高度
      var avatarurl_x = 11;   //绘制的头像在画布上的位置
      var avatarurl_y = 11;   //绘制的头像在画布上的位置

      this.ctx.save();
      console.log(this)
      if(this.text=="avatar"){
        //准备切割头像成圆形
        this.ctx.arc(avatarurl_width / 2 + avatarurl_x, avatarurl_heigth / 2 + avatarurl_y, avatarurl_width / 2, 0, Math.PI * 2, false);
        this.ctx.fill(0, 0, 128, 128)
        this.ctx.clip();
      }
        // TODO 剪切
        ...
        if (this.selected) {
            ...
            if (this.type === 'text') {
                ...
                this.ctx.drawImage('./icon/close.png', textX - 10, textY - 10, 20, 20);
                this.ctx.drawImage('./icon/scale.png', textX + textWidth - 15, textY + textHeight - 15, 20, 20);
            } else {
                this.ctx.strokeRect(this.x, this.y, this.w, this.h);
                this.ctx.drawImage('./icon/close.png', this.x - 10, this.y - 10, 20, 20);
                this.ctx.drawImage('./icon/scale.png', this.x + this.w - 15, this.y + this.h - 15, 20, 20);
            }
        }
